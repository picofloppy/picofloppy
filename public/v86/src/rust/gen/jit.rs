    ...
    0xDA | 0x1DA => {
        ...
        match modrm_byte >> 3 & 7 {
            0 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_0_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_0_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            1 => {
                codegen::gen_task_switch_test(ctx);
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_1_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_1_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            2 => {
                codegen::gen_task_switch_test(ctx);
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_2_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_2_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            3 => {
                codegen::gen_task_switch_test(ctx);
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_3_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_3_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            4 => {
                codegen::gen_task_switch_test(ctx);
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_4_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_4_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            5 => {
                codegen::gen_task_switch_test(ctx);
            ...
            6 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_6_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_6_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            7 => {
                codegen::gen_task_switch_test(ctx);
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DA_7_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DA_7_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            _ => {
                codegen::gen_trigger_ud(ctx);
    ...
    0xDB | 0x1DB => {
        ...
        match modrm_byte >> 3 & 7 {
            ...
            1 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DB_1_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DB_1_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            2 => {
                codegen::gen_task_switch_test(ctx);
    ...
    0xDD => {
        ...
        match modrm_byte >> 3 & 7 {
            ...
            1 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr16_DD_1_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr16_DD_1_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            2 => {
                codegen::gen_task_switch_test(ctx);
    ...
    0x1DD => {
        ...
        match modrm_byte >> 3 & 7 {
            ...
            1 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr32_DD_1_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr32_DD_1_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            2 => {
                codegen::gen_task_switch_test(ctx);
    ...
    0xDF | 0x1DF => {
        ...
        match modrm_byte >> 3 & 7 {
            ...
            1 => {
                ...
                if modrm_byte < 0xC0 {
                    let addr = modrm::decode(ctx.cpu, modrm_byte);
                    jit_instructions::instr_DF_1_mem_jit(ctx, addr);
                }
                else {
                    jit_instructions::instr_DF_1_reg_jit(ctx, (modrm_byte & 7) as u32);
                }
            },
            2 => {
                codegen::gen_task_switch_test(ctx);
