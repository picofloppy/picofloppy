...
match opcode {
    ...
    0x0B | 0x10B => {
        jit_instructions::instr_0F0B_jit(ctx);
        *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
    },
    0x0C | 0x10C => {
        codegen::gen_move_registers_from_locals_to_memory(ctx);
    ...
    0x12 | 0x112 => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_F20F12_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_F20F12_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else if ctx.cpu.prefixes & prefix::PREFIX_F3 != 0 {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_F30F12_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_F30F12_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else {
            codegen::gen_task_switch_test_mmx(ctx);
    ...
    0x16 | 0x116 => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_660F16_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_660F16_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
        }
        else if ctx.cpu.prefixes & prefix::PREFIX_F3 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_F30F16_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_F30F16_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_0F16_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_0F16_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
    },
    0x17 | 0x117 => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_660F17_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_660F17_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
        }
        else {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_0F17_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_0F17_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
                *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
        }
    },
    ...
    0x7C | 0x17C => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_660F7C_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_660F7C_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else if ctx.cpu.prefixes & prefix::PREFIX_F2 != 0 {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_F20F7C_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_F20F7C_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else {
            codegen::gen_move_registers_from_locals_to_memory(ctx);
    ...
    0x7D | 0x17D => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_660F7D_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_660F7D_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else if ctx.cpu.prefixes & prefix::PREFIX_F2 != 0 {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                jit_instructions::instr_F20F7D_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32);
            }
            else {
                jit_instructions::instr_F20F7D_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32);
            }
        }
        else {
            codegen::gen_move_registers_from_locals_to_memory(ctx);
    ...
    0xC4 | 0x1C4 => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_660FC4_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32, imm);
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_660FC4_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
        }
        else {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_0FC4_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32, imm);
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_0FC4_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
        }
    },
    0xC5 | 0x1C5 => {
        ...
        if ctx.cpu.prefixes & prefix::PREFIX_66 != 0 {
            ...
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_660FC5_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32, imm);
                *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_660FC5_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
        }
        else {
            codegen::gen_task_switch_test_mmx(ctx);
            if modrm_byte < 0xC0 {
                let addr = modrm::decode(ctx.cpu, modrm_byte);
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_0FC5_mem_jit(ctx, addr, (modrm_byte >> 3 & 7) as u32, imm);
                *instr_flags |= jit::JIT_INSTR_BLOCK_BOUNDARY_FLAG;
            }
            else {
                let imm = ctx.cpu.read_imm8() as u32;
                jit_instructions::instr_0FC5_reg_jit(ctx, (modrm_byte & 7) as u32, (modrm_byte >> 3 & 7) as u32, imm);
            }
        }
    },
    0xC6 | 0x1C6 => {
